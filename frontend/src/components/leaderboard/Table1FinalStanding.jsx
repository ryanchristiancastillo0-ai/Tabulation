...
export default  function Table1FinalStandings({ standings, isRankMode, onFullscreen, onCSV, onPNG }) {
  return (
    <section>
      <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-[#1B4332] text-white text-[10px] sm:text-xs font-bold px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-sm hover:opacity-90 transition-all"
          >
            <FullscreenIcon />
      ...
      <div id="table-standings" className="bg-white border border-[#BBCABB] rounded-sm overflow-hidden">
        {/* Banner */}
        <div className="bg-gradient-to-r from-[#1B4332] to-[#2D6A4F] px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between flex-wrap gap-2">
              ...
              {standings.length === 0 ? (
                ...
                >
                  ...
                  <td className="px-3 sm:px-6 py-3 sm:py-5">
                    <div className="font-bold text-[#14201A] text-sm sm:text-lg leading-tight">{c.name}</div>
                    {idx === 0 && (
                  ...
                  <td className="px-3 sm:px-6 py-3 sm:py-5 text-right">
                    <span className={`font-mono font-black text-lg sm:text-2xl ${idx === 0 ? 'text-[#1B4332]' : 'text-[#14201A]'}`}>
