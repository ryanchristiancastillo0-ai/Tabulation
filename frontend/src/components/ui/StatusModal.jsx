...
function ModalShell({ isOpen, title, message, type, onClose, footer }) {
...
const closeBtnCls = 'w-full py-2.5 text-sm text-[#4B5A4D] font-semibold rounded-sm border border-[#E1E8DE] hover:bg-[#FBFCF9] active:scale-95 transition-all';

export function SuccessModal({ isOpen, title, message, onClose, onViewScoreboard }) {
  return (
    <ModalShell
      isOpen={isOpen}
      title={title}
      ...
      footer={
        <div className="mt-7 flex flex-col gap-2 w-full">
          <a
            href={onViewScoreboard ? undefined : getJudgePageUrl('/judge/scoreboard', getSchoolId())}
...
export function ErrorModal({ isOpen, title, message, onClose }) {
  return (
    <ModalShell
      ...
      footer={
        <div className="mt-7 flex flex-col gap-2 w-full">
          <button onClick={onClose} className={`${closeBtnCls} text-rose-600 border-rose-200 hover:bg-rose-50`}>
            Close
          </button>
        </div>
      }
...
export function WarningModal({ isOpen, title, message, onClose }) {
  return (
    <ModalShell
      ...
      footer={
        <div className="mt-7 flex flex-col gap-2 w-full">
          <button onClick={onClose} className={`${closeBtnCls} text-amber-600 border-amber-200 hover:bg-amber-50`}>
            Close
          </button>
        </div>
      }
...
export function ConfirmModal({ isOpen, title, message, onClose, onConfirm }) {
  return (
    <ModalShell
      isOpen={isOpen}
      title={title}
